...
## Handoff from #0022 (asset pipeline)

The asset zip generated by `npm run assets` (see #0022 status) contains both block AND item textures + models:

- `assets/minecraft/textures/item/*.png` (793 files in 26.1.2)
- `assets/minecraft/models/item/*.json` (1285 files)

These are extracted by `fetch-assets.ts` but **not** loaded into the world atlas — `frontend/src/world/atlas.ts` explicitly filters textures to `textures/block/`. Build your item-icon atlas as a separate pipeline:

- The current code in `world/atlas.ts` re-fetches `/mc-assets.zip` per `loadBlockProviders()` call (which is itself a cached singleton). For the inventory panel, factor out a small `loadAssetZip()` helper that returns the parsed `JSZip` and have both `world/atlas.ts` and `panels/itemIcons.ts` consume it — the browser caches the HTTP response, but you don't want to parse the zip twice.
- For deepslate's `ItemRenderer` you'll need item models flattened against block models (item → parent `block/cube_all` etc.). Both subtrees are present in the zip; the same `BlockModelProvider` shape works.
- The shared `MeshStandardMaterial` from `world/blockAdapter.ts` is for the world view only — don't reuse for item icons. Render icons offscreen via `ItemRenderer` and bake to a sprite sheet, per the issue plan.
- `mc-assets.zip` is gitignored; README has a "Minecraft assets" section explaining how to generate it. Missing-zip error surfaces (world pane and inventory) should share a single toast once #0033 lands.
